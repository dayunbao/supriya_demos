...
## 🔁 Signal Flow (High-Level Structure)
...
## 🎯 Differences from the Original Dattorro Algorithm
...
4.  **Simplified Negation:**
  ...
  [STEREO IN] 
     ...
     [PRE-DELAY & FILTERS] (Input HPF/LPF)
           │
           ▼ (8x AllpassN in Series)
  ╔═════════════════════════════════════════════════════════╗
  ║                 INPUT DIFFUSION STAGE                   ║
  ╚═════════════════════════════════════════════════════════╝
           │ (Diffused Mono Signal)
           ▼
  ...
  ▼                    ▼
╔══════════════════╗ ╔══════════════════╗
║  LEFT TANK INPUT ║ ║ RIGHT TANK INPUT ║
║ (Blended w/ R FB)║ ║ (Blended w/ L FB)║
╚══════════════════╝ ╚══════════════════╝
    │                      │
    ▼                      ▼
...
[LOCAL OUT (L)] │  │         [LOCAL OUT (R)]
    ...
    ▼                                   ▼
┌──────────────────┐ ┌──────────────────┐
│ L-TANK OUTPUT TAPS │ │ R-TANK OUTPUT TAPS │
│ (8 Taps from L&R)  │ │ (8 Taps from L&R)  │
└──────────────────┘ └──────────────────┘
    │                      │
    ▼                      ▼
...
def plateau_reverb(
    ...
    # Scale tank diffusion: 0-10 → 0-0.7
